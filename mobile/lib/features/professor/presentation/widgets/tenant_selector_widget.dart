...
final professorTenantsProvider =
    FutureProvider.autoDispose<List<TenantModel>>((ref) async {
...
class TenantSelectorWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Only show selector if professor has multiple tenants
    return tenantsAsync.when(
      data: (tenants) {
        if (tenants.length <= 1) {
          return const SizedBox.shrink();
        }
        return _buildSelectorCard(context, ref, colorScheme, currentTenantId, currentTenant, tenantsAsync);
      },
      loading: () => _buildSelectorCard(context, ref, colorScheme, currentTenantId, currentTenant, tenantsAsync),
      error: (error, stackTrace) => _buildSelectorCard(context, ref, colorScheme, currentTenantId, currentTenant, tenantsAsync),
    );
  }
