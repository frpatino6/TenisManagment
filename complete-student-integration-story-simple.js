...
async function completeStudentIntegrationStory() {
  try {
    ...
    // First, get the current issue to see its state
    const getIssueQuery = `
      query {
        issue(identifier: "TEN-74") {
          id
          identifier
          title
          state {
    ...
    const issueResponse = await makeLinearRequest(config, getIssueQuery);
    const issue = issueResponse.data?.issue;
    
    if (!issue) {
      console.log('❌ No se encontró la historia TEN-74');
      return;
    }

    console.log(`📋 Historia encontrada: ${issue.identifier}: ${issue.title}`);
    console.log(`📊 Estado actual: ${issue.state.name}`);

    // Find the "Done" state
    const doneState = issue.team.states.nodes.find(state => 
      state.name.toLowerCase() === 'done' || state.name.toLowerCase() === 'completed'
    );

    if (!doneState) {
      console.log('❌ No se encontró el estado "Done"');
      console.log('Estados disponibles:', issue.team.states.nodes.map(s => s.name));
      return;
    }
