...
export class MongoGroupStageRepository implements GroupStageRepository {
    private readonly logger = new Logger({ service: 'MongoGroupStageRepository' });
    ...
    async update(groupStage: GroupStage): Promise<GroupStage> {
        this.logger.debug('Actualizando GroupStage', {
            tournamentId: groupStage.tournamentId,
            categoryId: groupStage.categoryId,
            groupsCount: groupStage.groups.length
        });

        // Log detallado de cada grupo antes de actualizar
        groupStage.groups.forEach((group, idx) => {
            this.logger.debug(`Grupo ${idx} a guardar`, {
                id: group.id,
                name: group.name,
        ...
        if (!updated) {
            this.logger.error('GroupStage no encontrado para actualizar', {
                tournamentId: groupStage.tournamentId,
                categoryId: groupStage.categoryId
            });
        ...
        this.logger.info('GroupStage actualizado exitosamente', {
            tournamentId: updated.tournamentId.toString(),
            categoryId: updated.categoryId
        });

        // Log de verificación después de guardar
        updated.groups.forEach((group: any, idx: number) => {
            this.logger.debug(`Grupo ${idx} guardado`, {
                id: group.id,
                participantsCount: group.participants.length,
