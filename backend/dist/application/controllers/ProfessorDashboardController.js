    ...
    constructor() {
        ...
        this.getMyTenants = async (req, res) => {
            ...
            try {
                ...
                const tenantsWithActivity = await Promise.all(professorTenants.map(async (pt) => {
                    const tenant = pt.tenantId;
                    // Get last schedule/booking for this professor in this tenant
                    const lastSchedule = await ScheduleModel_1.ScheduleModel.findOne({
                        professorId: professor._id,
                        tenantId: tenant._id,
                    })
                        .sort({ startTime: -1 })
