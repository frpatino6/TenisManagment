...
// repositories
container.bind<ProfessorRepository>(TYPES.ProfessorRepository).toConstantValue(new MongoProfessorRepository());
container.bind<StudentRepository>(TYPES.StudentRepository).toConstantValue(new MongoStudentRepository());
container.bind<ScheduleRepository>(TYPES.ScheduleRepository).toConstantValue(new MongoScheduleRepository());
container.bind<BookingRepository>(TYPES.BookingRepository).toConstantValue(new MongoBookingRepository());
...
container.bind<ManageCourtAvailabilityUseCase>(TYPES.ManageCourtAvailabilityUseCase).toDynamicValue(ctx => new ManageCourtAvailability(ctx.container.get(TYPES.ScheduleRepository)));
container.bind<TrackIncomeUseCase>(TYPES.TrackIncomeUseCase).toDynamicValue(ctx => new TrackIncome(ctx.container.get(TYPES.ReportRepository)));
container.bind<ManageServicesUseCase>(TYPES.ManageServicesUseCase).toDynamicValue(ctx => new ManageServices(ctx.container.get(TYPES.ServiceRepository)));
container.bind<BookLessonUseCase>(TYPES.BookLessonUseCase).toDynamicValue(ctx => new BookLesson(ctx.container.get(TYPES.BookingRepository), ctx.container.get(TYPES.ScheduleRepository)));
